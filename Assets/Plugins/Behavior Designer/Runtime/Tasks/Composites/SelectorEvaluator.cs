...
{
    [TaskDescription("选择器评估器是一个选择器任务，它每次都会重新评估它的子任务。" +
                     "它将运行最低优先级的子进程，并返回一个正在运行的任务状态。" +
                     "每隔一秒就会这样做。" +
                     "如果一个高优先级的子线程正在运行，下一帧一个低优先级的子线程想要运行，它将中断高优先级的子线程。" +
                     "选择器评估器将在第一个子进程返回成功时立即返回成功，否则它将继续尝试更高优先级的子进程。" +
                     "这个任务模拟除了子任务之外，条件中止功能并不总是条件任务。" +
                     "The selector evaluator is a selector task which reevaluates its children every tick. It will run the lowest priority child which returns a task status of running. " +
